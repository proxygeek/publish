...
do
	...
	do
		...
		textfile=$(find "$filedir/$filename".post "$filedir/$filename".md ! -path "$filedir" -print -quit 2>/dev/null) #modified
		...
		#	((z++))
		# done < <(echo "${section_colors[section_index]}")

		# backgroundcolor=$(echo "${section_colors[section_index]}" | sed -n 2p)
		# if [ "$override_textcolor" = false ]
		# then
			# textcolor=$(echo "${section_colors[section_index]}" | tail -1)
		# fi
		...
		do
			...
			then
				...
				then
					section_image_options[section_index]="$value"
				fi

				if [ "$key" = "video-options" ]
				then
					section_video_options[section_index]="$value"
				fi

				if [ "$key" = "video-filters" ]
				then
					section_video_filters[section_index]="$value"
				fi
			fi
		done < <(echo "$metadata")
