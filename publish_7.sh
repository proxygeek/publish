...
# Change Log:
# -----------------  Carried Over --------------------
# 06. Fix the post excerpts formating on section pages: DONE
# 06.1: Also implemented a workaround for top-level index.html by creating "About" section:
##       Problem: Script picks up files alphabetically - and treats the first file it finds as index.html. About's a workaround
# ---------------Planned in this version --------------
# 11. TODO: Create a link to the about-me post 
# 03. TODO: Fix the top level index.html
# -----------------  Some day --------------------
# 04. TODO: Refactor the code to remove redundant logic and restructure for clarity
# 07. TODO: Fix post tag search / links , especially in case of multiple tags for a post
# 08. TODO: Generate section level tag cloud from individual post tags: dir_tag_cloud
# 09. TODO: Number of (and links to) top posts to be read from config file in the top directory
# 10. TODO: Pick up related posts from YAML. say related_posts:post1,post2 -->
# ==============================================================================
...
template () {
	# remove spaces from the 2nd argument to template function call
	key=$(echo "$2" | tr -d '[:space:]')
	
	# replace all "/" in $3 (3rd argument) with "\/" 
...
# '
	...
	nav_type+=("$node_type")
#done < <(find "$topdir" -type d ! -path "$topdir*/_*" | sort)  # alphabetical sort
done < <(find "$topdir" -type d ! -path "$topdir*/_*" -printf "%T@ %Tc %p\n" | sort -nr | cut -d ' ' -f 9)  # sort by modification date

# find . -printf "%T@ %Tc %p\n" | sort -nr   # To sort by most recently updated instead of alphabetically
...
do
	...
	done < <(find "$dir" -maxdepth 1 ! -path "$dir" ! -path "$dir*/_*" -printf "%T@ %Tc %p\n" | sort -nr | cut -d ' ' -f 9)	# sort by modification date
	# done < <(find "$dir" -maxdepth 1 ! -path "$dir" ! -path "$dir*/_*" | sort) # sort alphabetically
...
do
	...
	section_metadata=""
	if [ -e "${paths[i]}/$metadata_file" ]  # -e returns true if the argument exists; checking if the metadata file exists for this dir
	then
	...
	do
		...
		# post=$(template "$post" post-content "$content")
		...
		if [ -z "$firsthtml" ]		# -z checks if the argument is null; it's checking if $firsthtml has been initialised
		then
			firsthtml="$html"
			firstpath="${nav_url[i]}"
		fi
